   ...
   <body>
      ...
      <form method="post">
       ...
       <h1>Modificar Médico</h1>
        ...
        <div class="btn-block">
          <button type="submit" class="hvr-pulse-shrink" name="registrationButton">Modificar</button>
        </div>
        <?php
         error_reporting(0);
         ...
         $post = (isset($_POST['dato_id']) && !empty($_POST['dato_id'])) &&
                 (isset($_POST['dato_nombre']) && !empty($_POST['dato_nombre'])) &&
         ...
         if($post)
         {
            ...
            if($fila = mysqli_fetch_array($resultado))
            {
               ...
               {
                  $connect->query("UPDATE medico SET nombre = '$dato_nombre', especialidad = '$dato_especialidad', correo = '$dato_correo',celular = '$dato_celular' WHERE id =  $dato_id");
                  echo "<script> swal.fire({
                  icon: 'success',
                  title: '¡Confirmación!',
                  text: 'El médico fue modificado correctamente',
                  showConfirmButton: false,
                  position: 'center',
                  toast: true,
                  allowOutsideClick: false,
